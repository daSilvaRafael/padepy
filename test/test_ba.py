...
class ba_wrong_input(unittest.TestCase):
    def test_input_obj(self):
        ba_pade12, ba_path12 = ba.pade(1, 2,var, "2")
        self.assertEqual(ba_pade12, None)
        self.assertEqual(ba_path12, None)
        func = [1, 2, "f", 3, 4]
        ba_pade55 = ba.pade(5, 5, var, func)
    ...
    def test_complex_number(self):
        func = [1, 2, sp.I, 3, 4]
        ba_pade22 = ba.pade(2, 2, var, func)
        self.assertEqual(ba_pade22, None)

    def test_nn_coeff(self):
        func = [1, 2, 3, 4, 5]
        ba_pade55 = ba.pade(3, 3, var, func)
        self.assertEqual(ba_pade55, None)

    def test_string_inputs(self):
        ba_pade8S, ba_path8S = ba.pade(8, "q", var, sp.exp(var))
        self.assertEqual(ba_pade8S, None)
        self.assertEqual(ba_path8S, None)
        ba_padeS10, ba_pathS10 = ba.pade("p", 10, var, sp.exp(var))
        self.assertEqual(ba_padeS10, None)
        self.assertEqual(ba_pathS10, None)
        ba_pade11, ba_path11 = ba.pade(1, 1, var, sp.exp(var), "d")
        self.assertEqual(ba_pade11, None)
        ba_pade11, ba_path11 = ba.pade(1, 1, var, sp.exp(var), "16")
        self.assertEqual(ba_pade11, None)


class ba_bad_output(unittest.TestCase):
    def test_cos(self):
        pade14, path = ba.pade(1, 4, var, sp.cos(var))
        self.assertEqual(pade14, None)

    def test_epx_1cos_16(self):
        func = sp.exp(var) + (1/sp.cos(var))
        pade019, path = ba.pade(0, 19, var, func, 16)
        self.assertEqual(pade019, None)        
    
    def test_epx_1cos_16(self):
        func = sp.exp(var) + (1/sp.cos(var))
        pade021, path = ba.pade(0, 21, var, func, 16)
        self.assertEqual(pade021, None)   


class ba_correct_output(unittest.TestCase):
    def test_exp(self):
        pade00, path00 = ba.pade(0, 0, var, sp.exp(var))
        self.assertEqual(pade00, 1)
        self.assertEqual(path00, 1)
        pade05, path05 = ba.pade(0, 5, var, sp.exp(var))
    ...
    def test_exp_fp_16(self):
        pade03, path03 = ba.pade(0, 3, var, sp.exp(var), 16)
        self.assertEqual(f"{pade03}", "1.0/(-0.1666666666666667*x**3 + 0.5*x**2 - 1.0*x + 1.0)")
        pade33, path33 = ba.pade(3, 3, var, sp.exp(var), 16)
        ...
        pade32, path32 = ba.pade(3, 2, var, sp.exp(var), 16)
